...
export default function StickyCta({
  ctaLabel = "Get prices & availability",
  modalEvent,
}: {
  ctaLabel?: string;
  /** When set, the CTA dispatches this window event (to open a modal)
      instead of anchoring to #get-prices. */
  modalEvent?: string;
}) {
  const [show, setShow] = useState(false);
  const openModal = modalEvent
    ? () => window.dispatchEvent(new Event(modalEvent))
    : undefined;
        ...
        <div className="flex items-stretch gap-2 px-3 py-2.5">
          ...
          {openModal ? (
            <button
              type="button"
              onClick={openModal}
              className="btn-gold flex-1 !py-3 text-center !text-[0.72rem]"
            >
              {ctaLabel}
            </button>
          ) : (
